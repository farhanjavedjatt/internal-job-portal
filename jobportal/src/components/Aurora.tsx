        ...
        <defs>
          {blobs.map((b, i) => (
            <radialGradient key={i} id={`blob-${i}`} cx="50%" cy="50%" r="50%">
              <stop
                offset="0%"
                stopColor={`oklch(${dark ? 65 : 80}% 0.18 ${b.hue})`}
                stopOpacity={dark ? 0.55 : 0.75}
              />
              <stop
                offset="60%"
                stopColor={`oklch(${dark ? 55 : 75}% 0.12 ${b.hue})`}
                stopOpacity={dark ? 0.25 : 0.35}
              />
              <stop
                offset="100%"
                stopColor={`oklch(${dark ? 50 : 70}% 0.08 ${b.hue})`}
                stopOpacity="0"
              />
            </radialGradient>
          ))}
