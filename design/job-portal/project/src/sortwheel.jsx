// Sort wheel — rotate-to-select, physical detented feel
const { useState: useStateSW, useEffect: useEffectSW, useRef: useRefSW } = React;

function SortWheel({ options, value, onChange }) {
  const [open, setOpen] = useStateSW(false);
  ...
  return (
    <div className={`sortwheel ${open ? 'is-open' : ''}`}>
      <button className="sortwheel-trigger" onClick={() => setOpen(!open)}>
        ...
        <svg width="10" height="10" viewBox="0 0 10 10" style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)' }}>
            ...
            {options.map((opt, i) => {
              ...
              return (
                <button
                  ...
                  style={{ transform: `rotate(${angle}deg) translateY(-78px) rotate(${-angle}deg) rotate(${-rotation}deg)` }}
                  onClick={() => { onChange(opt.id); setTimeout(() => setOpen(false), 280); }}
                >
...
Object.assign(window, { SortWheel });
