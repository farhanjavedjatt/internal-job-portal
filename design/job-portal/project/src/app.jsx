...
function App({ tweaks, setTweaks }) {
  ...
  const filtered = useMemoApp(() => {
    let list = jobs.filter(j => {
      ...
      if (filters.sizes.length && !filters.sizes.includes(j.companySize)) return false;
    ...
    else if (sort === 'applicants') sorted.sort((a,b) => b.applicants - a.applicants);
    return sorted;
  }, [jobs, filters, sort]);
      ...
      <div className="app">
        ...
        <header className="topbar glass">
          <div className="brand">
            <div className="brand-mark" aria-hidden>
              <svg width="28" height="28" viewBox="0 0 28 28">
                <circle cx="14" cy="14" r="11" fill="none" stroke="url(#brand-g)" strokeWidth="1.5"/>
            ...
            <div className="brand-name">Prism<span className="brand-dot">.</span>work</div>
          </div>
          ...
          <div className="search glass-inset">
            ...
            />
            {filters.q && (
              <button className="search-clear" onClick={() => setFilters(f => ({ ...f, q: '' }))}>×</button>
            )}
          ...
          <div className="topbar-right">
            ...
            <button className="theme-toggle" onClick={() => setTweaks(t => ({ ...t, theme: t.theme === 'dark' ? 'light' : 'dark' }))} aria-label="toggle theme">
              {tweaks.theme === 'dark' ? (
                <svg width="18" height="18" viewBox="0 0 18 18"><circle cx="9" cy="9" r="3.5" fill="currentColor"/><g stroke="currentColor" strokeWidth="1.2" strokeLinecap="round">{[0,45,90,135,180,225,270,315].map(a=>(<line key={a} x1={9 + Math.cos(a*Math.PI/180)*6} y1={9 + Math.sin(a*Math.PI/180)*6} x2={9 + Math.cos(a*Math.PI/180)*7.5} y2={9 + Math.sin(a*Math.PI/180)*7.5}/>))}</g></svg>
              ) : (
                <svg width="18" height="18" viewBox="0 0 18 18"><path d="M13 10 A5 5 0 0 1 8 5 A5 5 0 0 0 13 10 Z" fill="currentColor"/></svg>
              )}
            </button>
            {editMode && (
              <button className="theme-toggle" onClick={() => setShowTweaks(s => !s)} aria-label="tweaks">
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="1.4">
                  <circle cx="9" cy="9" r="2"/><path d="M9 2 V4 M9 14 V16 M2 9 H4 M14 9 H16 M4 4 L5.5 5.5 M12.5 12.5 L14 14 M4 14 L5.5 12.5 M12.5 5.5 L14 4"/>
                </svg>
              </button>
            )}
          </div>
        ...
        <div className="layout">
          ...
          <main className="main">
            ...
            <div className="stats-strip glass">
              ...
              <div className="stat">
                <div className="stat-num">${heroStats.medianSalary}<span style={{fontSize:'0.5em', opacity: 0.5}} className="mono">k</span></div>
            ...
            <div className="results-head">
              <div className="results-title">
                ...
                {filters.q && <span className="results-q"> · matching "<em>{filters.q}</em>"</span>}
              </div>
              <div className="density-toggle">
                <button
                  ...
                  aria-label="spacious"
                >
                  <svg width="14" height="14" viewBox="0 0 14 14"><rect x="1" y="1" width="5.5" height="5.5" rx="1" fill="currentColor"/><rect x="7.5" y="1" width="5.5" height="5.5" rx="1" fill="currentColor"/><rect x="1" y="7.5" width="5.5" height="5.5" rx="1" fill="currentColor"/><rect x="7.5" y="7.5" width="5.5" height="5.5" rx="1" fill="currentColor"/></svg>
                </button>
                <button
                  ...
                  aria-label="dense"
                >
                  <svg width="14" height="14" viewBox="0 0 14 14"><rect x="1" y="2" width="12" height="1.5" rx="0.5" fill="currentColor"/><rect x="1" y="6.3" width="12" height="1.5" rx="0.5" fill="currentColor"/><rect x="1" y="10.5" width="12" height="1.5" rx="0.5" fill="currentColor"/></svg>
                </button>
              </div>
            </div>
            ...
            {filtered.length === 0 ? (
              <div className="empty glass">
                ...
                <div className="empty-sub mono">Loosen a filter or clear them all to see the full set.</div>
              </div>
        ...
        {showTweaks && editMode && <TweaksPanel tweaks={tweaks} setTweaks={setTweaks} onClose={() => setShowTweaks(false)} />}
      </div>
    </>
  );
}
